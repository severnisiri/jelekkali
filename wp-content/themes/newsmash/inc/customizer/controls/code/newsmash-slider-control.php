...
/**
 * Class NewsMash_Customizer_Range_Control
 *
 * @access public
 */
class NewsMash_Customizer_Range_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_script(
			'newsmash-controls',
			NEWSMASH_THEME_INC_URI . '/customizer/controls/js/controls.js',
			array(
				'jquery',
				'customize-base',
				'jquery-ui-button',
		...
		wp_enqueue_style( 'newsmash-controls', NEWSMASH_THEME_INC_URI . '/customizer/controls/css/controls.css' );
		wp_enqueue_style( 'font-awesome', NEWSMASH_THEME_URI . '/assets/vendors/css/all.css' );
	}
	
	/**
	 * NewsMash_Customizer_Range_Control constructor.
	 *
 ...
 */
function newsmash_sanitize_range_value( $input ) {
	if ( newsmash_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function newsmash_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
