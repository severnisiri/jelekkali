...
class Image{
	...
	static function revert_all(){
		...
		foreach($images->posts as $key => $post){
			...
			if(wp_next_scheduled('speedycache_img_delete', array($post->ID))){
				continue;
			}
			...
			if(!wp_next_scheduled('speedycache_img_delete', array($post->ID))){
				wp_schedule_single_event($final_schd_time, 'speedycache_img_delete', array($post->ID));
					...
					<table style="margin:auto; width: 100%;">
						...
						<tbody>
							...
							foreach($scheduled as $schedule){
								...
								?>
							<tr>
								<td class="speedycache-table-item"><?php echo esc_html($schedule['name']); ?></td>
